...
public class YaoYiYao : EnemyController
{
    ...
    protected override void DecideIntent()
    {
        ...
        if (isYang)
        {
            ...
            currentIntent = EnemyIntent.Attack;
            intentValue = 6 + _strength;
        }
        else
        {
            // 阴卦：造成 4 + 力量 伤害，获得 10 格挡
            currentIntent = EnemyIntent.Attack;
            intentValue = 4 + _strength;
        }
    }
    ...
    protected override void ExecuteIntent()
    {
        ...
        if (_isBurstTurn)
        {
            Debug.Log($"{enemyData.enemyName} 连续三卦相同！大招！造成 {intentValue} 点伤害（25 + 力量{_strength}）");
        }
        else
        {
            ...
            {
                _strength += 3;
                Debug.Log($"{enemyData.enemyName} 卜得阳卦！造成 {intentValue} 点伤害，力量 +3，当前力量 {_strength}");
            }
            else
            {
